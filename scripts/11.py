...
#create 5 joint for the actual binded joint and snap them to the locator
	...
	pm.addAttr(longName='IKFKswitchIK', defaultValue=0.0, minValue=0.0, maxValue=1.0 )
	pm.setAttr(ControlCurve[0] + '.IKFKswitchIK',e = True, keyable = True)
	pm.addAttr(longName='IKFKswitchFK', defaultValue=0.0, minValue=0.0, maxValue=1.0 )
	...
	#stretchy
	stretchyLocator1 = pm.spaceLocator("IKstretchLocator1")
	stretchyLocator2 = pm.spaceLocator("IKstretchLocator2")
	print(stretchyLocator1)
	

	pm.pointConstraint(IKHandler[0],stretchyLocator2,w = 1,offset = [0,0,0])
	pm.pointConstraint(IKjointGroup[JOINT_NUMBER-1],stretchyLocator1,w = 1,offset = [0,0,0])
	distanceBetween = pm.createNode( "distanceBetween")

	pm.connectAttr( stretchyLocator2+ ".translate", distanceBetween + ".point2")
	pm.connectAttr( stretchyLocator1+ ".translate", distanceBetween + ".point1")

	conditionNode = pm.createNode("condition")

	pm.connectAttr( distanceBetween+ ".distance", conditionNode + ".colorIfFalseR")
	pm.connectAttr( distanceBetween+ ".distance", conditionNode + ".secondTerm")
	#setAttr "condition4.operation" 3;
	pm.setAttr(conditionNode + ".operation",3)

	locatorLoc1 = pm.getAttr(stretchyLocator1 + ".translate")
	locatorLoc2 = pm.getAttr(stretchyLocator2 + ".translate")
	distance = ((locatorLoc1[0] - locatorLoc2[0])**2 + (locatorLoc1[1] - locatorLoc2[1])**2 + (locatorLoc1[2] - locatorLoc2[2])**2)**(1/2.0)
	
	#setAttr "condition7.firstTerm" 5.1;
	#setAttr "condition7.colorIfTrueR" 5.1;
	pm.setAttr(conditionNode + ".firstTerm",distance * 1.01)
	pm.setAttr(conditionNode + ".colorIfTrueR",distance * 1.01)
	
	multiplDivide = pm.createNode( "multiplyDivide")
	pm.setAttr(multiplDivide + ".input2X", distance)
	pm.setAttr(multiplDivide + ".operation", 2)
	
	#connectAttr -f condition9.outColorR multiplyDivide2.input1X;
	pm.connectAttr( conditionNode+ ".outColorR", multiplDivide + ".input1X")
	#connectAttr -f multiplyDivide3.outputX IK_leg3_jnt.scaleY;
	pm.connectAttr( multiplDivide+ ".outputX", IKjointGroup[JOINT_NUMBER-2] + ".scaleY")
	pm.connectAttr( multiplDivide+ ".outputX", IKjointGroup[JOINT_NUMBER-1] + ".scaleY")
